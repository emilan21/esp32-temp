...
static esp_err_t http_rest_with_url(const char *post_data) {
  ...
  while (1) {
    ...
    if (dht11_err != 0) {
      ...
      snprintf(post_data, sizeof(post_data),
               ...
               "\"%.2f\"}",
               DEVICE_ID, c_to_f(dht11_sensor.temp), dht11_sensor.humidity);
      if (cnt % POST_EVERY_N_READS == 0) {
